...
# make tidy -> Question number as column
centroids
```

```{r, fig.width = 10, fig.height = 8}
ggplot(centroids) + 
  geom_col(aes(value, reorder(question, value))) +
  facet_wrap(~ reorder(cluster, -value))

library(scico)
ggplot(centroids) +
  geom_tile(aes(
    reorder(question, value, var), 
    reorder(cluster, value), 
    fill = value,
    col = value
    )
    ) +
  scale_fill_scico("bamO", midpoint = 0.5) +
  scale_color_scico("bamO", midpoint = 0.5)
...
bind_cols(cluster = factor(fit$cluster), survey) |>
  count(cluster, `WBL Report Year`) |>
  ggplot() +
    geom_col(aes(`WBL Report Year`, n, fill = factor(cluster)))
