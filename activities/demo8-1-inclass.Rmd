...
```{r}
ggraph(G) +
    geom_edge_diagonal(color = "#d3d3d3", linewidth = 2) +
    geom_node_point(aes(color = date), size = 5)
```

```{r}
G <- G |>
    mutate(country_new = fct_lump_n(country, 7))

ggraph(G) +
    geom_edge_diagonal(color = "#d3d3d3", linewidth = 2) +
    geom_node_point(aes(color = country_new), size = 5) +
    scale_color_brewer(palette = "Set2", na.value = "#5d5858") +
    facet_wrap(~ cut(date, 6)) +
