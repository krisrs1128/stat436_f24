...
data_dir <- path("/Users/krissankaran/Downloads/subset/")
ix <- which(py$y == 1)[1:10]
img <- brick(data_dir / py$fnames[1])
tm_shape(img) + tm_rgb()

imgs <- map(py$fnames[ix], ~ brick(data_dir / .))
...
umap_recipe <- recipe(~., data = data.frame(py$h)) |>
  step_umap(all_predictors()) |>
  prep()

embeddings <- bake(umap_recipe, new_data = NULL)
embeddings <- embeddings |>
  mutate(class = factor(py$y))

```

```{r, fig.width = 10, fig.height = 10}
ggplot(embeddings, aes(UMAP1, UMAP2, color = class)) +
  geom_point(size = 20) +
  labs(title = "UMAP Embeddings of Histopathology Images",
       x = "UMAP1",
       y = "UMAP2")
```
...
embeddings <- embeddings |>
  mutate(file = data_dir / py$fnames)

ggplot(embeddings) +
  geom_image(aes(UMAP1, UMAP2, image = file))
